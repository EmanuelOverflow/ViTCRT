...
from lib.train.models.vit_crt.backbone import FrozenBatchNorm2d
...
class Corner_Predictor(nn.Module):
    ...
    def _max_batch_safe(self, x, axes=(1, 2), return_argmax=False):
        max = x
        argmax = -1
        for axis in reversed(axes):
            max, argmax = torch.max(max, axis)
    ...
    def _min_batch_safe(self, x, axes=(1, 2), return_argmin=False):
        min = x
        argmin = 9999
        for axis in reversed(axes):
            min, argmin = torch.min(min, axis)
    ...
    def _layer_activation(self, features_map):
        lam = features_map.sum(1)

        lam_max = self._max_batch_safe(lam).view(-1, 1, 1)
        lam_min = self._min_batch_safe(lam).view(-1, 1, 1)
        lam = (lam - lam_min) / lam_max

        lam_max = self._max_batch_safe(lam).view(-1, 1, 1)
        f_am = torch.where(lam >= (lam_max * .2), lam, torch.zeros_like(lam))
        return f_am[0]
    ...
    def forward(self, x, return_dist=False, softmax=True):
        ...
        if return_dist:
            ...
            return torch.stack((coorx_tl, coory_tl, coorx_br, coory_br), dim=1) / self.img_sz, None, None  #, \
                  # self._layer_activation(score_map_tl), self._layer_activation(score_map_br)
...
def build_box_head(cfg):
    if cfg.MODEL.HEAD_TYPE == "MLP":
        ...
        cls_head = MLP(hidden_dim, hidden_dim, 4, 3)  # dim_in, dim_hidden, dim_out, 3 layers
        return cls_head
        ...
        if cfg.MODEL.BACKBONE.DILATION is False:
            ...
            stride = 8
        feat_sz = int(cfg.DATA.SEARCH.SIZE / stride)  # Depthwise: 13
        feat_sz = int(feat_sz / getattr(cfg.MODEL, "PATCH_SIZE", 1))
...
def build_cls_head(cfg):
    if cfg.MODEL.HEAD_CLS_TYPE == "MLP":
        hidden_dim = cfg.MODEL.HIDDEN_DIM
        cls_head = MLP(hidden_dim, hidden_dim, 2, 3)  # dim_in, dim_hidden, dim_out, 3 layers
        return cls_head
    elif cfg.MODEL.HEAD_CLS_TYPE == "BOX_MIX":
        hidden_dim = cfg.MODEL.HIDDEN_DIM
        cls_head = MLP(hidden_dim, hidden_dim, 2, 3)  # dim_in, dim_hidden, dim_out, 3 layers
        return cls_head
    elif cfg.MODEL.HEAD_CLS_TYPE == "None":
        return None
    else:
        raise ValueError("HEAD TYPE %s is not supported." % cfg.MODEL.HEAD_CLS_TYPE)
