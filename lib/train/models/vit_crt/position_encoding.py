...
class VitPositionalEncoding(nn.Module):
    def __init__(self, patch_size, embed_dim, in_channels=256, masking=False):
        super(VitPositionalEncoding, self).__init__()
        if not masking:
            # Rearrange -> unfold(kernel_size=4, stride=4)
            patch_dim = in_channels * patch_size * patch_size
            self.patch_embed = Rearrange('b c (h p1) (w p2) -> b (h w) (p1 p2 c)', p1=patch_size, p2=patch_size)
        else:
            patch_dim = in_channels
            self.patch_embed = Rearrange('b c (h p1) (w p2) -> b (p1 p2 h w) c', p1=patch_size, p2=patch_size)
            # self.patch_embed = torch.nn.Unfold(kernel_size=patch_size, stride=patch_size)
        self.proj_embed = nn.Linear(patch_dim, embed_dim)

    def forward(self, tensor_list: NestedTensor):
        patch = self.patch_embed(tensor_list.tensors)
        return self.proj_embed(patch)
...
def build_position_encoding(cfg):
    ...
    elif cfg.MODEL.POSITION_EMBEDDING == 'vit':
        position_embedding = VitPositionalEncoding(patch_size=cfg.MODEL.PATCH_SIZE,
                                                   embed_dim=cfg.MODEL.HIDDEN_DIM,
                                                   in_channels=cfg.MODEL.BACKBONE.OUTPUT_DIM,
                                                   masking=cfg.MODEL.TRANSFORMER.MASKING)
    else:
