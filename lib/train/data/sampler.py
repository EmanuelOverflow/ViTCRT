...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            if is_video_dataset:
                ...
                elif self.frame_sample_mode == "vit_cr":
                    template_frame_ids, search_frame_ids = self.get_frame_ids_stark(visible, seq_info_dict["valid"])
                elif self.frame_sample_mode == "dinardo":
                    template_frame_ids, search_frame_ids = self.get_frame_ids_dinardo(visible)
                else:
    ...
    def get_one_search(self):
        ...
        # sample a frame
        if is_video_dataset:
            if self.frame_sample_mode == "vit_cr":
    ...
    def get_frame_ids_stark(self, visible, valid):
        # get template and search ids in a 'vit_cr' manner
        template_frame_ids_extra = []
        while None in template_frame_ids_extra or len(template_frame_ids_extra) == 0:
            template_frame_ids_extra = []
            # first randomly sample two frames from a video
    ...
    def get_frame_ids_dinardo(self, visible):
        gap_increase = 0
        search_frame_ids = None
        template_frame_ids = None
        while search_frame_ids is None:
            ...
            search_frame_ids = [template_frame_ids[0] + 1]
            next_template_id = self._sample_visible_ids(visible, min_id=search_frame_ids[0] + 1,
                                                        max_id=search_frame_ids[0] + self.max_gap + gap_increase,
                                                        num_ids=self.num_search_frames)
            if next_template_id is None:
                gap_increase += 5
                continue

            template_frame_ids.extend(next_template_id)
            # Increase gap until a frame is found
            gap_increase += 5
        return template_frame_ids, search_frame_ids
