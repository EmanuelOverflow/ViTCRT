...
class ViTCRTProcessing(BaseProcessing):
    """ The processing class used for training ViTCRT. The images are processed in the following way.
...
class DINARDOProcessing(BaseProcessing):
    ...
    def __init__(self, search_area_factor, output_sz, center_jitter_factor, scale_jitter_factor,
                 mode='pair', settings=None, cfg=None, *args, **kwargs):
        """
        args:
            search_area_factor - The size of the search region  relative to the target size.
            output_sz - An integer, denoting the size to which the search region is resized. The search region is always
        ...
        self.mode = mode
        self.settings = settings
        self.size = cfg.DATA.TEMPLATE.SIZE
        self.search_size = cfg.DATA.SEARCH.SIZE
        self.stride = 8
        self.grid_to_search_x, self.grid_to_search_y = self.grids()
    ...
    def __call__(self, data: TensorDict):
        ...
        for s in ['template', 'search']:
            ...
            for ele in data[s + '_att']:
                if (ele == 1).all():
                    ...
                    return data
            for ele in data[s + '_att']:
                feat_size = self.output_sz[s] // 8 + 1  # 16 is the backbone stride
                # (1,1,128,128) (1,1,256,256) --> (1,1,8,8) (1,1,16,16)
        ...
        if data["template_masks"] is None or data["search_masks"] is None:
            data["template_masks"] = torch.zeros((1, self.output_sz["template"], self.output_sz["template"]))
            data["search_masks"] = torch.zeros((1, self.output_sz["search"], self.output_sz["search"]))
            data['cls_score'] = torch.zeros((1, 25, 25))

        if isinstance(data['template_masks'], list):
            ele = data['template_masks'][0]
            output_sz = 25
            cls_score = F.interpolate(ele[None, None].float(), size=output_sz)[0]
            cls_score = torch.where(cls_score >= 0.5, 1., 0.)
            data['cls_score'] = [cls_score]

        if isinstance(data['template_anno'], list):
            print(data['template_anno'][0])
            reg_label, reg_weight = self.reg_label(data['template_anno'][0])
            data['reg_label'] = reg_label
    ...
    def grids(self):
        ...
        grid_to_search_x = x * self.stride + self.search_size // 2
        grid_to_search_y = y * self.stride + self.search_size // 2
