...
def masks_to_bboxes(mask, fmt='c'):
    ...
    for m in mask:
        mx = m.sum(dim=-2).nonzero(as_tuple=False)
        my = m.sum(dim=-1).nonzero(as_tuple=False)
...
def masks_to_bboxes_multi(mask, ids, fmt='c'):
    ...
    for id in ids:
        mx = (mask == id).sum(dim=-2).nonzero(as_tuple=False)
        my = (mask == id).float().sum(dim=-1).nonzero(as_tuple=False)
        bb = [mx.min(), my.min(), mx.max(), my.max()] if (len(mx) > 0 and len(my) > 0) else [0, 0, 0, 0]
