...
# from lib.train.models.vit_cr.stark_s import build_starks
...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='stark_st2', choices=['vit_cr', 'stark_st2'],
...
def evaluate(model, search, seq_dict, run_box_head, run_cls_head):
    ...
    # # backbone
    macs1, params1 = profile(model, inputs=(search, None, "backbone", False),
                             custom_ops=None, verbose=False)
    macs, params = clever_format([macs1, params1], "%.3f")
    print('backbone (search) macs is ', macs)
    print('backbone params is ', params)
    # transformer and head
    macs2, params2 = profile(model, inputs=(None, seq_dict, "transformer", False),
    ...
    with torch.no_grad():
        # overall
        for i in range(T_w):
            _ = model(search, None, "backbone", False)
            _ = model(None, seq_dict, "transformer", False, False)
        start = time.time()
        for i in range(T_t):
            _ = model(search, None, "backbone", False)
            _ = model(None, seq_dict, "transformer", False, False)
...
if __name__ == "__main__":
    ...
    '''import vit_cr network module'''
    model_module = importlib.import_module('lib.train.models.vit_cr')
    if args.script == "vit_cr":
        model_constructor = model_module.build_vitcr
        ...
        seq_dict = merge_template_search_vit([oup_t, oup_s])
